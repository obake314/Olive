...
router.get('/', (req: AuthRequest, res: Response) => {
  ...
  const todos = db.prepare(
    `SELECT t.*, u.name as created_by_name FROM todos t
...
router.post('/', (req: AuthRequest, res: Response) => {
  ...
  const todo = db.prepare('SELECT t.*, u.name as created_by_name FROM todos t JOIN users u ON u.id = t.user_id WHERE t.id = ?').get(id);
...
router.patch('/:id/toggle', (req: AuthRequest, res: Response) => {
  ...
  res.json(db.prepare('SELECT t.*, u.name as created_by_name FROM todos t JOIN users u ON u.id = t.user_id WHERE t.id = ?').get(todo.id));
});
...
router.put('/:id', (req: AuthRequest, res: Response) => {
  ...
  const { title, due_date } = req.body;
  if (!title?.trim()) return res.status(400).json({ error: 'タイトルは必須です' }) as any;
  db.prepare('UPDATE todos SET title = ?, due_date = ? WHERE id = ?').run(title.trim(), due_date || null, req.params.id);
  res.json(db.prepare('SELECT t.*, u.name as created_by_name FROM todos t JOIN users u ON u.id = t.user_id WHERE t.id = ?').get(req.params.id));
});
